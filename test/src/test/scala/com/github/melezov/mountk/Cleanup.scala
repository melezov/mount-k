...
object Cleanup:
  private val ProdRegSubPath = "SYSTEM\\CurrentControlSet\\Control\\Session Manager\\DOS Devices"
  private val TestRegRoot = s"HKCU\\$TestRegSubPath"
  private val ProdRegRoot = s"HKLM\\$ProdRegSubPath"

  private val DriveLetterName = """^([A-Za-z]):$""".r
  private val SubstTargetPrefix = "\\??\\"

  def main(args: Array[String]): Unit =
    println("mount-k cleanup")

    val liveSubsts = ('A' to 'Z').filter(WinApi.isSubst).toList
    val substDeferred = liveSubsts.filter { c =>
      if WinApi.substDelete(c) then
        println(s"  unmounted $c:")
        false
      else
        println(s"  $c: subst belongs to another LUID, deferring to elevated pass")
        true
    }
    if liveSubsts.isEmpty then println("  no subst mappings to clear")

    val prodLetters = WinApi.regQueryValues(WinReg.HKEY_LOCAL_MACHINE, ProdRegSubPath).collect {
  ...
  private def runDirect(bat: Path): Unit =
    ...
    if rc != 0 then println(s"  cleanup script returned exit $rc")
  ...
  private def runElevated(bat: Path, substLetters: List[Char], prodLetters: List[Char]): Unit =
    ...
    println(s"  elevation needed ($reasons); requesting UAC...")
    def psQuote(s: String): String = "'" + s.replace("'", "''") + "'"
    val ps =
      s"try { $$p = Start-Process -Verb RunAs -Wait -PassThru -WindowStyle Hidden " +
      s"-FilePath ${psQuote(bat.toString)} -ErrorAction Stop; exit $$p.ExitCode } " +
      s"catch { exit 1223 }"
    Process(Seq("powershell", "-NoProfile", "-Command", ps)).! match
      case 0    => println("  elevated cleanup completed")
      case 1223 => println("  UAC prompt cancelled; entries left in place")
      case n    => println(s"  elevated cleanup failed (exit $n)")
